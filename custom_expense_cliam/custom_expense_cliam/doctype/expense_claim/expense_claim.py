...
class ExpenseClaim(AccountsController):
	...
	def get_exchange_rate (self):
			...
			self.conversion_rate = get_exchange_rate(self.currency,company_currency,transaction_date=self.posting_date)
			for i in getattr(self,'expenses',[]) :
				i.currency = self.currency
			#for i in getattr(self,'taxes',[]):
			#	i.currency = self.currency
			advances = get_advances(employee=self.employee,currency=self.currency)
	...
	def get_gl_entries(self):
		...
		if self.grand_total:
			...
			payable_account_rate = get_exchange_rate( self.currency,payable_account_currency)
			gl_entry.append(
				self.get_gl_dict({
					"account": self.payable_account,
					"credit": self.grand_total * self.conversion_rate,
					"account_currency": payable_account_currency ,
					"credit_in_account_currency": self.grand_total * payable_account_rate,
		...
		for data in self.expenses:
			default_account_currency = get_account_currency(data.default_account)
			default_account_rate = data.conversion_rate or get_exchange_rate( self.currency,default_account_currency)
			gl_entry.append(
				self.get_gl_dict({
					"account": data.default_account,
					"debit": data.sanctioned_amount * self.conversion_rate,
					"account_currency": default_account_currency ,
					"debit_in_account_currency": data.sanctioned_amount * default_account_rate,
		...
		for data in self.advances:
			advance_account_currency = get_account_currency(data.advance_account)
			advance_account_rate = self.conversion_rate or get_exchange_rate( self.currency,advance_account_currency)
			gl_entry.append(
				self.get_gl_dict({
					"account": data.advance_account,
					"credit": data.allocated_amount * advance_account_rate,
     				"account_currency": advance_account_currency ,
					"credit_in_account_currency": data.allocated_amount * advance_account_rate,
					"against": ",".join([d.default_account for d in self.expenses]),
					"party_type": "Employee",
					"party": self.employee,
		...
		if self.is_paid and self.grand_total:
			...
			payment_account_rate = get_exchange_rate( self.currency,payment_account_currency)
			gl_entry.append(
				self.get_gl_dict({
					"account": payment_account,
					"credit": self.grand_total * self.conversion_rate,
     				"account_currency": payment_account_currency ,
					"credit_in_account_currency": self.grand_total * payment_account_rate,
					"against": self.employee
				}, item=self)
			)
			#payable_account_currency = get_account_currency(self.payable_account)
			...
			gl_entry.append(
				self.get_gl_dict({
					...
					"debit": self.grand_total * self.conversion_rate,
     				"account_currency": payable_account_currency ,
					"debit_in_account_currency": self.grand_total * payable_account_rate,
					"against_voucher": self.name,
					"against_voucher_type": self.doctype,
				}, item=self)
			)
		#for i in gl_entry :
		#		frappe.msgprint(""" {}  =>   {}    :    {}""".format(i.account , i.debit_in_account_currency , i.credit_in_account_currency))
		#		frappe.msgprint(""" {}  =>   {}    :    {}""".format(i.account , i.debit , i.credit))
	...
	def add_tax_gl_entries(self, gl_entries):
		...
		for tax in self.get("taxes"):
			account_head_currency = get_account_currency(tax.account_head)
			account_head_rate = get_exchange_rate( self.currency,account_head_currency)
			gl_entries.append(
				self.get_gl_dict({
					"account": tax.account_head,
					"debit": tax.tax_amount * self.conversion_rate,
					"account_currency": account_head_currency ,
					"debit_in_account_currency": tax.tax_amount * account_head_rate,
					"against": self.employee,
					"cost_center": self.cost_center,
					"against_voucher_type": self.doctype,
					"against_voucher": self.name
...
@frappe.whitelist()
def get_advances(employee, advance_id=None , currency=None):
	con = ""
	if currency:
		con = " and currency = '{}' " .format(currency)
	if not advance_id:
		condition = 'docstatus=1 and employee={0} {1} and paid_amount > 0 and paid_amount > claimed_amount + return_amount'.format(frappe.db.escape(employee), frappe.db.escape(con))
	else:
...
@frappe.whitelist()
def get_account_details (expense_claim_type ,currency, company):
    data = get_expense_claim_account(expense_claim_type, company)
    account_currency = get_account_currency(data.get('account'))
    rate = get_exchange_rate(currency,account_currency) or 0
    return {
...
def get_expense_claim(
	...
	expense_claim.currency = doc.currency
	expense_claim.conversion_rate = doc.exchange_rate
